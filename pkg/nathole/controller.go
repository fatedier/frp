...
func (c *Controller) HandleVisitor(m *msg.NatHoleVisitor, transporter transport.MessageTransporter) {
	...
	time.Sleep(time.Duration(cResp.DetectBehavior.ReadTimeoutMs+10000) * time.Millisecond)
}
...
func (c *Controller) HandleReport(m *msg.NatHoleReport) {
	...
	c.analyzer.Report(session.analysisKey, session.recommandMode, session.recommandIndex, m.Success)
...
func (c *Controller) analysis(session *Session) (*msg.NatHoleResp, *msg.NatHoleResp, error) {
	...
	log.Debug("sid [%s] visitor nat %+v, candidateAddrs %v; client nat %+v, candidateAddrs %v",
		session.sid, *vNatFeature, vm.MappedAddrs, *cNatFeature, cm.MappedAddrs)
