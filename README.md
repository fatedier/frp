...
| ➖ Bỏ | `http+https` (khi đã có SSL thì không ai dùng http nữa) |
...
Type sugar `http+https` đã bị **gỡ hoàn toàn** (cùng toàn bộ cơ chế combo-sugar).
Khai `type = "http+https"` giờ báo lỗi *unknown proxy type*. Khi cần cả hai, khai
riêng 1 proxy `http` và 1 proxy `https` — nhưng thường có SSL rồi thì chỉ cần `https`.

---

## 3. Mặc định `wireProtocol = v2`
...
## 4. Sửa lỗi kẹt reconnect ở v2 (issue #5355)
