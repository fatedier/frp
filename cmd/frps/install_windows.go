...
var installCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			fmt.Println("frps: the executable no longer exists")
			os.Exit(1)
		}
		stat, err := os.Stat(execPath)
		if err != nil || stat.IsDir() {
			fmt.Println("frps: the executable is no longer valid")
			os.Exit(1)
		}
...
func verifyCfg(f string) error {
	svrCfg, _, err := config.LoadServerConfig(f, strictConfigMode)
	if err != nil {
		return err
	}
...
func installService(exec string, args ...string) error {
	scm, err := mgr.Connect()
	if err != nil {
		fmt.Println("frps: Failed connect to SCM (Permission Denied)")
		return err
	}
	defer func() {
		_ = scm.Disconnect()
	...
	_, err = scm.CreateService("frps", exec, mgr.Config{
...
func modifyService(scm *mgr.Mgr, exec string, args ...string) error {
	service, err := scm.OpenService("frps")
	if err != nil {
		return err
	}
