...
// The frps side of xudp is identical to xtcp: frps only brokers the NAT hole
// signaling (it never sees the UDP payload), so this is a straight copy of the
// xtcp server proxy with the config type swapped. writeNatHoleSid is shared with
// the xtcp server proxy (same package).
